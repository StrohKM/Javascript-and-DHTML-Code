...
//get maximum number of tests from the value of the max attribute from the <input> element with id='numscores', 
			// maximum = 1;
maximum = parseInt($('numscores').getAttribute('max')); 
...
//if user input for number of test scores is valid and in the range of 1 to maximum, then complete step 1.2
			if (numInputs >= 1 && numInputs <= maximum) 
			{   
...
//or you may use appendChild() method to append new elements one by one, 		
					document.getElementById('testInputs').append(label, input, span, br);						
				}
...
//convert user input into numerical data: you may use parseFloat() method
				let inputElement = document.getElementById(`test${i}`);
				let spanElement = inputElement.nextElementSibling; // assumes span is directly after input
...
//if a test score is valid, add that test score to the score array.		
			if (isNaN(inputValue) || inputValue < 0 || inputValue > 150) 
			{
				spanElement.textContent = "Invalid input. Enter a number between 0 and 150.";
				inputElement.classList.add('error');
			} 
			else 
			{
				score.push(inputValue);
			}
		} //end of the for loop 
...
//If not all inputted test scores are valid, then create an alert box to display an error message 	
			if(score.length === numInputs)
			{
				...
				let finalGrade;
				if (avgScore >= 120 && avgScore <= 150 ) finalGrade = "A";
				else if (avgScore >= 100 && avgScore < 120) finalGrade = "B";
				else if (avgScore >= 80 && avgScore < 100) finalGrade = "C";
				else if (avgScore >= 60 && avgScore < 80) finalGrade = "D";
				else finalGrade = "F";
