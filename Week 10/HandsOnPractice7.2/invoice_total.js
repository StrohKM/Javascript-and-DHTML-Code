...
let processEntries = function () {
	...
	if (isNaN(invoiceSubtotal) || invoiceSubtotal < 0.0) {
		...
		[percent, discount, total].forEach(ele => localStorage.setItem(ele.id, ele.value));
...
// }
// Better way to make writeToStorage function
const writeToStorage = () =>
{
	[type, subtotal, percent, discount, total].forEach(
...
// Rewriting retrieveData function to be easier to read
const retrieveData = () => 
{
	[type, subtotal, percent, discount, total].forEach(
		ele => ele.value = localStorage.getItem(ele.id) ?? ''); //if cannot find the data in local storage, assign empty string
...
[type, subtotal].forEach(ele => ele.addEventListener('change', function()
{
	writeToStorage(); //Update local storage
	processEntries(); //Update calculations
}));
...
window.onload = function () {
	...
	// the web page
	if(localStorage.getItem(type.id)) //if customer type data is stored in local storage
	{
		// reload page, data existing
		alert("reload page");
		// Getting data since customer type was found
		retrieveData();
	}
	else
	{
		alert("First time opening page");
		// No data was found so write data that was inputted into the storage
		writeToStorage();
	}
