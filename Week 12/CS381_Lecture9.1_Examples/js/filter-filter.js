$(function() {
 ...
 //  Nigel: {
  ...
  function priceRange2(person, index) { 
   //how to find the index of the person in the array whose rate within the range?
	if(person.rate >= 65 && person.rate <= 120)
	{
		console.log(index);
		return true;
	}
  ...
  // filter() calls priceRange(), if priceRange() returns true, then the member in 
  // the "people" array will be returned from filter method 
